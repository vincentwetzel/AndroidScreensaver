...
- Added `CODING_STANDARDS.md` for Kotlin/Android engineering rules, review expectations, security/privacy guidance, dependency hygiene, and verification policy.
- Moved detailed coding standards out of `ARCHITECTURE.md` so architecture documentation stays focused on system design.
- Updated documentation indexes and build requirements to include the coding standards file and the current `compileSdk` value.
- Aligned app Gradle version metadata with the in-progress `1.11.0` changelog entry.
...
- **OneDrive integration** - Added OneDrive as a full photo source with hybrid authentication: MSAL for phones/tablets and Microsoft device code flow for Android TV. OneDrive sources now appear on the main screen, support folder browsing, re-authentication, recursive subfolder traversal, and local cache downloads.
...
- **BaseAccountManager** — Abstracted common logic and state management out of `GoogleAccountManager` and `DropboxAccountManager` into a unified generic base class.
...
- **Per-account folder browsing** - `FolderBrowserActivity` and `CloudFolderViewModel` now route all cloud folder API calls to the correct source and account via intent extras.
...
- **Folder Browser Crash** - Fixed a critical `NullPointerException` in `FolderBrowserActivity` where navigating into a cloud source folder crashed the app because `dreamSourceType` was accessed before being extracted from intent extras.
- **DataStore Save Race Condition** - Refactored `FolderAdapter` to emit a single unified `onSelectionStateChanged` callback instead of separate checked/unchecked callbacks. This prevents two concurrent read-modify-write operations on DataStore when clicking a checkbox, avoiding corrupted or dropped folder selections.
- **Multi-Account UI Overwrite** - Removed legacy singleton observers (`observeViewModel`) from `MainActivity` that were incorrectly overwriting the status text of all cloud accounts with the name of the last accessed account, strictly adhering to the per-account remote operation standard.
- **Lifecycle-aware flow collection** - Refactored `GalleryFolderBrowserActivity` to collect ViewModel state flows using `repeatOnLifecycle` instead of direct coroutine launches, preventing background memory leaks when the UI is not visible.
- **UI string compliance** - Replaced generic "Authenticated" status text in `MainActivity` with "Signed in as Unknown" fallback to strictly comply with design system guidelines, and added the missing "Successfully signed in" success toast to the Dropbox authentication flow.
- **Multi-Account Folder Interface Trap** - Standardized the `PhotoRepository` contract by adding the `accountId` parameter to `listFolders` and `searchFolders`. This eliminates the final "First Available Account" fallback trap, ensuring that generic consumers (like `SlideshowManager`) can safely list and search folders across secondary Google Drive and Dropbox accounts without encountering cross-account API errors.
- **Gallery subfolder traversal bottleneck** - Fixed a critical performance bug in `GalleryPhotoRepository.getSubfolderIds()` where finding subfolders caused the app to load the device's entire MediaStore library into memory to perform manual string-prefix matching. The filtering is now handled natively by SQLite using a `LIKE` clause, drastically improving cascade-selection speed on devices with thousands of photos.
- **Settings deserialization data corruption** - Fixed a critical bug in `SettingsManager` where `email`, `selectedFolders`, and `deselectedFolders` were correctly URL-encoded during serialization but never decoded during JSON parsing. This caused emails to display incorrectly (e.g. `user%40gmail.com`) and broke folder ID matching entirely.
- **Search result race conditions** - Fixed an issue in `GalleryViewModel` and cloud folder browsing where rapidly typing in the folder search bar would launch multiple unmanaged coroutines. Slower, older search results could overwrite newer results; active search and load operations are now strictly cancelled when a new request is made.
- **Google Drive Multi-Account State Loss (Critical Bug)** - Fixed an issue where `GoogleAccountManager` relied exclusively on `GoogleSignIn.getLastSignedInAccount()` at startup, causing any secondary or tertiary Google accounts to permanently disappear from the app's memory whenever the device rebooted or the app restarted. The manager now tracks all authenticated emails in preferences and transparently reconstructs their `Drive` API services via `GoogleAccountCredential`.
- **Main Menu Account Name Blanking** - Fixed a UI bug in `MainViewModel` where the Google Drive account email would show as blank on startup if the underlying `GoogleSignInAccount` object wasn't available. It now correctly queries the email directly from the repository's tracked ID.
- **Weather API 400 Bad Request** - Fixed a silent network failure in `WeatherRepository` where requesting `precipitation_probability` in the `current` Open-Meteo forecast block caused the API to reject the request. The weather overlay will now correctly fetch and display live conditions.
- **Datastore account ID corruption** - Fixed a vulnerability in `SettingsManager` where a custom account ID containing a pipe `|` or semicolon `;;` delimiter would fatally break the deserialization loop. Account IDs are now strictly URL-encoded alongside emails and paths.
- **Dropbox malformed root path crash** - Fixed a path generation bug in `DropboxPhotoRepository` where referencing the Dropbox root folder `""` resulted in an invalid path formatted as `"/"`, crashing the API `listFolder` builder.
- **Gallery compilation error** - Fixed an unresolved reference to `totalCount` in `GalleryPhotoRepository` that broke the build.
- **Dropbox prefetching ignored** - Fixed a missing override in `DropboxPhotoRepository` that caused all Dropbox background cache prefetching requests to route to an empty base method and fail silently.
- **Media Type Filter Mismatch (Silent Bug)** - Fixed a major silent bug where the Content Type filter (Images Only / Videos Only) was completely ignored during photo loading. The settings manager passed uppercase enum names (`IMAGES_ONLY`) but the repositories checked for lowercase string aliases (`images`), causing all source queries to incorrectly fall back to the default `Both` state. Filters and their cache keys are now strictly normalized in all repositories.
- **Gallery Sibling Folder Collision** - Fixed a path-prefix bug where counting or listing photos recursively in a folder (e.g., `Pictures/Cats`) would accidentally pull in media from identically-prefixed sibling folders (e.g., `Pictures/Cats_Old`) by enforcing trailing slashes on MediaStore `RELATIVE_PATH` queries.
- **DataStore corruption from special characters** - Fixed a critical settings serialization bug where Dropbox folders containing commas or pipes in their names (e.g., "Vacation, 2023") would break the `SettingsManager` parsing logic, erasing saved accounts. Folder paths and emails are now safely URL-encoded before persisting.
- **Filter state lost on navigation** - Fixed an issue in `GalleryViewModel` and cloud folder browsing where navigating back up the directory tree or clearing a search query would reset the visible photo count to include all media types, ignoring the active "Images/Videos Only" filter.
- **Lingering source enablement** - Fixed a UI state bug in `MainViewModel` where signing out of Google Drive or Dropbox failed to automatically untoggle the source's enabled switch.
- **Dead code cleanup** - Removed unused `getFolderPhotoCountForAccount` methods in cloud repositories (properly handled by the base class).
...
- **Multi-Account URL and Metadata Routing Trap** - Fixed a critical architectural flaw where fetching metadata, resolving URLs, or grabbing thumbnails via the generic `PhotoRepository` interface fell back to the *first available account*. Now, the interface explicitly accepts an optional `accountId` parameter which passes through to properly route secondary metadata fetching in `SlideshowManager` (eliminating `403 Forbidden` API crashes when displaying media from non-primary accounts).
...
- **Google Drive compilation error** - Fixed missing return statement in `GoogleDrivePhotoRepository.getThumbnailUrlForAccount` that caused a build failure.
- **FolderError usability** - Refactored `userMessage()` out of the companion object into a standard member function for cleaner access.
- **Google Drive repository stability** - Fixed cache key collisions from relying on `Set.hashCode()` for excluded folders, patched a bug where missing Drive filenames displayed as `"null.jpg"`, and added cycle detection to recursive subfolder and photo fetching to prevent infinite API loops.
- **Google Drive security and edge-cases** - Safely URL-encoded `accountId` parameters in Drive media URLs, sanitized disk cache filenames to remove illegal characters, and escaped folder IDs in API string-builders to prevent single-quote injection crashes during nested traversal. Fixed broken indentation from previous merges.
- **Google Drive null safety and privacy** - Prevented potential null pointer exceptions by explicitly dropping Drive items with missing IDs during API traversal, and replaced `e.printStackTrace()` with sanitized logging to strictly prevent OAuth tokens from leaking into logcat during network errors.
- **Dropbox Auth Rotation State Loss** - Fixed an issue in `DropboxAuthActivity` where rotating the device while the Dropbox OAuth browser was open would cause the activity to lose its `authStarted` state, silently ignoring the authentication result when the user returned to the app.
- **Dropbox Auth Error Path State** - Fixed a bug in `DropboxAuthActivity` where an exception thrown during the Dropbox sign-in result processing would fail to hide the loading progress bar and permanently disable the sign-in button, violating the error-path UI standard.
- **Debug Settings Exit Cancellation** - Fixed a vulnerability in the `DebugSettingsActivity` reset function where the app termination sequence could be cancelled by Activity destruction, leaving the app running with corrupted in-memory state. Termination is now safely wrapped in a `NonCancellable` context.
- **Dropbox Auth Client Leak** - Fixed a resource leak in `DropboxAuthActivity` where a new `OkHttpClient` instance was created on every authentication attempt. The client is now shared and its thread pools are aggressively shut down in `onDestroy()`.
- **Debug Settings Complete Exit Reliability** - Fixed a lingering vulnerability in `DebugSettingsActivity` where the reset function's UI context shift could still throw a `CancellationException` and halt app termination. The entire UI update and termination sequence is now firmly wrapped in a `NonCancellable` context.
- **Dropbox Auth Cancellation Handling** - Fixed an issue in `DropboxAuthActivity` where coroutine cancellation during the sign-in flow (e.g., from device rotation) was being incorrectly caught as a generic exception, resulting in a misleading "Authentication error: Job was cancelled" Toast and improper error handling.
- **Settings save lifecycle races** - Fixed several settings screens so synchronous UI state is captured before DataStore reads resume, preventing user changes from being mixed with stale state in decoration, photo info, schedule, cache, background color, and timeout settings.
- **Folder selection metadata stability** - Fixed Gallery and cloud folder browsers so selection saves preserve visible folder names/paths, snapshot selected and deselected IDs before async persistence, and keep summary counts stable across refreshes and navigation.
- **Folder browser refresh and auth states** - Fixed folder browser pull-to-refresh to reload the current folder without duplicating back-stack entries, tightened auth-error detection to typed `FolderError.AuthError`, and cleared UI refresh indicators on error.
