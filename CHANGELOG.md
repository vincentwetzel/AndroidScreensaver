...
- **Search result race conditions** - Fixed an issue in `GalleryViewModel` and `GoogleDriveViewModel` where rapidly typing in the folder search bar would launch multiple unmanaged coroutines. Slower, older search results could overwrite newer results; active search and load operations are now strictly cancelled when a new request is made.
...
- **Filter state lost on navigation** - Fixed an issue in `GalleryViewModel` and `GoogleDriveViewModel` where navigating back up the directory tree or clearing a search query would reset the visible photo count to include all media types, ignoring the active "Images/Videos Only" filter.
